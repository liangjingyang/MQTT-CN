...
这一章仅仅是一个指引，而且似乎非规范要求的。但是强烈推荐服务端实现提供TLS[RFC5246]，使用TCP协议8883端口（IANA服务名称：secure-mqtt）。

有一些风险是解决方案的提供者应该思考的，例如：
...
MQTT解决方案经常部署在通信恶略的环境。这种情况下，实现经常需要提供如下机制：
...
使用NIST网络安全框架的指引可以在MQTT的补充补充出版资料中找到，《MQTT和NIST改善关键基础设施网络安全框架》[MQTT NIST]。行业使用证明，独立的验证和认证可以帮助达到要求。
...
实现用明文传送认证数据，或混淆数据元素，或不要求认证都应该意识到，这样会有中间人攻击或重播攻击的风险。

5.4.5节介绍了确保数据私密性的方法。

客户端和服务端之间的虚拟私有网络可以保证只会从认证过的客户端收到数据。
...
如果使用TLS[RFC5246]，客户端可以用服务端发来的SSL证书来认证。如果实现提供一个ip地址的多个主机名的MQTT服务，应该注意RFC 6066[RFC6066]第3节定义的TLS服务器名扩展。允许客户端告知服务端尝试连接服务器的主机名。
...
使用VPN网络连接客户端和服务端可以保证数据在VPN端传输的私密性。
