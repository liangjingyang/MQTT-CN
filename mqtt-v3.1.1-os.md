...
一致性的服务端必须支持一个或多个能够提供有序，可靠，双向字节流的底层传输协议[MQTT-7.1.1-1]。但是一致性不依赖于任何特殊的传输层协议。服务端或许会支持4.2节所列的传输层协议，或者是其他符合要求的传输层协议[MQTT-7.7.7-2]。
...
一致性的客户端必须支持一个或多个能够提供有序，可靠，双向字节流的底层传输协议[MQTT-7.1.1-1]。但是一致性不依赖于任何特殊的传输层协议。客户端或许会支持4.2节所列的传输层协议，或者是其他符合要求的传输层协议[MQTT-7.7.7-2]。
